...
<div id="comments" class="comments-area">
	...
	<?php if (have_comments()) { ?>
		<h2 class="comments-title">
			<?php
			printf(_nx('One comment on &ldquo;%2$s&rdquo;', '%1$s comments on &ldquo;%2$s&rdquo;', get_comments_number(), 'comments title', 'voisen'), number_format_i18n(get_comments_number()), '<span>' . get_the_title() . '</span>');
			?> 
		</h2>

		<?php if (get_comment_pages_count() > 1 && get_option('page_comments')) { // are there comments to navigate through  ?> 
			<h3 class="screen-reader-text sr-only"><?php esc_html__('Comment navigation', 'voisen'); ?></h3> 
			<ul id="comment-nav-above" class="comment-navigation pager">
				<li class="nav-previous previous"><?php previous_comments_link(esc_html__('&larr; Older Comments', 'voisen')); ?></li>
				<li class="nav-next next"><?php next_comments_link(esc_html__('Newer Comments &rarr;', 'voisen')); ?></li>
		...
		<ul class="media-list">
			 ...
			 * to use voisen_bootstrap_comment() to format the comments.
			 * If you want to override this in a child theme, then you can
			 * define voisen_bootstrap_comment() and that will be used instead.
			 * See voisen_bootstrap_comment() in inc/template-tags.php for more.
			 */
			wp_list_comments(array('avatar_size' => '64', 'callback' => 'voisen_bootstrap_comment'));
			?>
		</ul><!-- .comment-list -->

		<?php if (get_comment_pages_count() > 1 && get_option('page_comments')) { // are there comments to navigate through  ?> 
			<h3 class="screen-reader-text sr-only"><?php esc_html_e('Comment navigation', 'voisen'); ?></h3>
	...
	if (!comments_open() && '0' != get_comments_number() && post_type_supports(get_post_type(), 'comments')) { ?> 
		<p class="no-comments"><?php esc_html_e('Comments are closed.', 'voisen'); ?></p>
	<?php 
	} //endif; 
	?> 
	...
	comment_form(
		array(
			'class_submit' => 'btn btn-primary',
				'fields' => array(
					'author' => '<div class="info-wrapper">' . 
								'<label class="control-label " for="author">' . esc_html__('Name', 'voisen') . ($req ? ' <span class="required">*</span>' : '') . '</label> ' .
					...
					'email'  => '<div class="info-wrapper">' . 
								'<label class="control-label" for="email">' . esc_html__('Email', 'voisen') . ($req ? ' <span class="required">*</span>' : '') . '</label> ' .
					...
					'url'    => '<div class="info-wrapper">' . 
								'<label class="control-label " for="url">' . esc_html__('Website', 'voisen') . '</label> ' .
			...
			'comment_field' => '<div class="message-wrapper">' . 
							'<label class="control-label " for="comment">' . esc_html__('Comment', 'voisen') . '</label> ' . 
			...
			'comment_notes_after' => '<p class="help-block">' . 
							sprintf(wp_kses(__('You may use these <abbr title="HyperText Markup Language">HTML</abbr> tags and attributes: %s', 'voisen'), array('abbr')), $comment_allowed_tags) . 
							'</p>'
		)
	); 
