...
<div class="main-container">
				...
				<div class="col-xs-12 col-md-<?php if ( $attachment_ids || has_post_thumbnail()) { echo '6';} else {echo '12';} ?>">
					...
					<div class="project-meta">
						<div class="project_date ">
							<p><strong><?php echo esc_html__('Date', 'voisen') ?></strong></p>
							<span class="day"><?php echo get_the_date(esc_html__('d', 'voisen')); ?></span>
							<span class="separator">/</span>
							<span class="month"><?php echo get_the_date(esc_html__('M', 'voisen')); ?></span>
						</div>
					
					
						<?php if (!empty($datagroup)) { ?>
						<div class="categories-entry">
							<p class="cat-label"><strong><?php echo esc_html__('Categories', 'voisen') ?></strong></p> <?php echo implode(", ", $datagroup); ?> 
						</div>
						...
						<div class="client-entry">
							<p class="client-label"><strong><?php echo esc_html__('Client', 'voisen') ?></strong></p> <?php echo esc_html($client); ?>
						</div>
						<?php } ?>
						<?php $url = esc_attr( get_post_meta( get_the_ID(), '_url', true ) ); ?>
						<?php if($url){ ?>
						<div class="url-entry">
							<p class="link-label"><strong><?php echo esc_html__('Link', 'voisen') ?></strong></p> <a href="<?php echo esc_url($url) ?>"><?php echo esc_html__('Visit project', 'voisen') ?></a>
						</div>
						<?php } ?>
						<?php $testimonial = esc_attr( get_post_meta( get_the_ID(), '_testimonials_search', true ) ); ?>
						<?php if($testimonial){ ?>
						<div class="testimonial-entry">
							<p class="testimonial-label"><strong><?php echo esc_html__('Testimonial', 'voisen') ?></strong></p> <?php echo esc_html($testimonial); ?>
						</div>
						<?php } ?>
						<div class="social-sharing clearfix">
							<?php $liked = voisen_check_liked_post(get_the_ID()); ?>
							<?php voisen_blog_sharing(); ?>
	...
	<?php
		...
		<div class="related_projects">
			<h3 class="related-title"><?php echo esc_html($voisen_opt['related_project_title']); ?></h3>
